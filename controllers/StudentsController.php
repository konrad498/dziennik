...
//            }
            ...
            $query = mysqli_query($db_connection,
                "SELECT students.id, students.name, students.surname, users.email FROM `database`.students JOIN `database`.users on `database`.`users`.`id` = `database`.`students`.`user_id` WHERE id = '{$_GET['student_id']}'");
